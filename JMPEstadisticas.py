...
class JMPEstadisticas():
    ...
    def calculoMediaAritmetica(self):
        ...
        n = self.caracteristica.count()#el metodo count no sirve para contar los elementos sirve para contar el num de vecces q aparece uno de ellos-->en las listas// en los pandas no: ESTA BIEN
        #n = self.caracteristica.str.len() # el metodo len ya implementado sí sirve para contar el numero de objetos pero estoy entre ese, size o el str.len de los pandas
    ...
    def calculoDelosCuartiles(self,mediana,rangoMediana):
        n = self.caracteristica.count()
        sort_caracteristica = self.caracteristica.sort_values(by='NOTAS') #el metodo sort_values admite el polimorfismo sobrecarga sin embargo el parametro "by=" tiene q estar definido siempre
        sort_caracteristica = sort_caracteristica.reset_index(drop=True)# eesta bien
        q1 = 0
        q2 = mediana
        q3 = 0
